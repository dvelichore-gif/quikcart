...
const CATS = ['Electronics','Home & Living','Wellness','Fashion','Sports','Books']
...
export default function Navbar() {
  ...
  const sn = { color:'rgba(255,255,255,0.85)',fontSize:12,padding:'0 10px',cursor:'pointer',whiteSpace:'nowrap',height:34,display:'inline-flex',alignItems:'center',borderRadius:4,textDecoration:'none' }
    ...
    <>
      ...
      <div style={{ background:'#1a1f2e',color:'rgba(255,255,255,0.65)',fontSize:11,padding:'5px 16px',display:'flex',alignItems:'center',gap:16,flexWrap:'wrap' }}>
        ...
        <span style={{ marginLeft:'auto',display:'flex',gap:14,alignItems:'center',position:'relative' }}>
          ...
          <span style={{ position:'relative' }}>
            <span onClick={() => setCurrOpen(o => !o)} style={{ color:'rgba(255,255,255,0.65)',cursor:'pointer' }}>
              {CURRENCIES[currency].symbol} {currency} ▾
            </span>
            {currOpen && (
              <div style={{ position:'absolute',top:'100%',right:0,marginTop:6,background:'white',borderRadius:8,boxShadow:'0 8px 24px rgba(0,0,0,0.25)',overflow:'hidden',minWidth:170,zIndex:200 }}>
                {Object.entries(CURRENCIES).map(([code, info]) => (
                  <div key={code} onClick={() => { changeCurrency(code); setCurrOpen(false) }}
                    style={{ padding:'9px 14px',fontSize:12,color:'#1a1f2e',cursor:'pointer',background:code===currency?'#e8f1fb':'white',fontWeight:code===currency?600:400 }}
      ...
      <nav style={{ background:'#1a6fc4',padding:'8px 16px',display:'flex',alignItems:'center',gap:10,position:'sticky',top:0,zIndex:100 }}>
        ...
        <form onSubmit={handleSearch} style={{ flex:1,display:'flex',maxWidth:580,borderRadius:6,overflow:'hidden',border:'2px solid #ff6b00',background:'white' }}>
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search products, brands and more..." style={{ flex:1,border:'none',padding:'0 10px',fontSize:13,outline:'none',color:'#1a1f2e' }} />
        ...
        <div style={{ display:'flex',alignItems:'center',gap:14,marginLeft:'auto',color:'white',fontSize:12,flexShrink:0 }}>
          ...
          <Link href="/cart" style={{ display:'flex',alignItems:'center',gap:6,background:'#ff6b00',padding:'6px 12px',borderRadius:6,color:'white',textDecoration:'none' }}>
            🛒 <span style={{ fontWeight:700,fontSize:14 }}>{totalItems}</span> <span style={{ fontSize:12 }}>Cart</span>
          </Link>
        </div>
      </nav>
      ...
      <div style={{ background:'#14549a',display:'flex',gap:2,padding:'0 16px',overflowX:'auto',height:34,alignItems:'center' }}>
        <span style={{ ...sn,cursor:'default',color:'rgba(255,255,255,0.6)' }}>☰ Categories</span>
        {CATS.map(cat => <Link key={cat} href={`/shop?cat=${encodeURIComponent(cat)}`} style={sn}>{cat}</Link>)}
        <Link href="/shop?badge=deal" style={{ ...sn,color:'#ff6b00' }}>⚡ Flash Deals</Link>
        <Link href="/shop" style={sn}>🚚 Free Shipping</Link>
      </div>
